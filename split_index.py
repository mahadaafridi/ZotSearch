...
class IndexSplitter:
    ...
    def initialize_total_documents(self, input_file: str) -> None:
        """
    ...
    def create_index_directory(self, base_dir: str) -> str:
    ...
    def calculate_tfidf(self, tf: float, df: int, fields: List[str]) -> float:
        ...
        """
        tf = 1+log(tf)
        idf = log(self.total_documents/(1+df))
        field_boost = 1
    ...
    def split_index(self, input_file: str, base_dir: str) -> None:
        """
        ...
        index_dir = self.create_index_directory(base_dir)
        ...
        with open(input_file, 'r') as f:
            for line in f:
                ...
                for posting in data['postings']:
                    posting['tfidf'] = self.calculate_tfidf(posting['tf'], df, posting['fields'])

                range_id = self.get_term_range(token)
